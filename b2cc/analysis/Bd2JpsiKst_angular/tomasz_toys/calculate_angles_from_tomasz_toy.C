...
#include "calculate_angles_from_tomasz_toy.h"
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
#include <TTree.h>
...
void calculate_angles_from_tomasz_toy::Loop()
{
    ...
    TNtuple * tuple = new TNtuple("tuple", "tuple",  "m23:phi:cosTheta2:cosTheta1:pionID");
	...
	if (jentry/1000 % 0) cout << jentry << endl;
        ...
        tuple->Fill(m23, _phi, cos_thetaK, cos_thetaL, -1);
    }
    tuple->Write();
    file->Close();
}
...
int main(){
    calculate_angles_from_tomasz_toy t;
    t.Loop();
    return 1;
