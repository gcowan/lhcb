{
	...
	TFile * f = TFile::Open("../../data/fitNTuple_peakBkg_2011_2012_Reco14_TOS_HLT2B_20140822_with_IPz_weights_v2.root");
	...
	TString cut = "(30. > TMath::Abs(phi_1020_M - 1020.)) && (16. > phi_1020_ENDVERTEX_CHI2/phi_1020_ENDVERTEX_NDOF) && (0.12 > B_s0_LOKI_DTF_CTAUERR/0.299792458) && (B_s0_LOKI_DTF_CHI2NDOF > 0) && (B_s0_LOKI_DTF_VCHI2NDOF > 0) && (5. > B_s0_LOKI_DTF_CHI2NDOF) && (10. > B_s0_ENDVERTEX_CHI2/B_s0_ENDVERTEX_NDOF) && (25. > B_s0_IPCHI2_OWNPV) && (B_s0_MINIPCHI2NEXTBEST > 50 || 0 > B_s0_MINIPCHI2NEXTBEST) && (60 > TMath::Abs(J_psi_1S_MM - 3090))&& (Kplus_PIDK > 0.) && (Kminus_PIDK > 0.)&&(B_s0_Hlt1TrackMuonDecision_TOS==1||B_s0_Hlt1TrackAllL0Decision_TOS==1) && (B_s0_Hlt2DiMuonDetachedJPsiDecision_TOS==1) && (B_s0_Hlt1DiMuonHighMassDecision_TOS==0)&&B_s0_LOKI_DTF_CTAU/0.2997>0.3";

	//time
		
	TH2D * h2 = new TH2D("h2", "h2", 100, 0.3, 16, 500, -1, 1);
	//DecayTree->Draw("1/B_velo_eff_weight_with_trigger:B_s0_LOKI_DTF_CTAU/0.2997>>h2", cut, "goff");
	//DecayTree->Draw("1/B_velo_eff_weight_with_track_trigger:B_s0_LOKI_DTF_CTAU/0.2997>>h2", cut, "goff");
	//DecayTree->Draw("1/B_velo_eff_weight_with_trigger:time>>h2", "sWeights_ipatia*(triggerDecisionUnbiased==1)", "goff");
	DecayTree->Draw("1/B_velo_eff_weight_with_track_trigger:time>>h2", "sWeights_ipatia*(triggerDecisionBiasedExcl==1)", "goff");
	TProfile * hprof = new TProfile("hprof", "hprof", 100, 0.3, 16, -1, 1);
	//DecayTree->Draw("1/B_velo_eff_weight_with_trigger:B_s0_LOKI_DTF_CTAU/0.2997>>hprof", cut, "goff");
	//DecayTree->Draw("1/B_velo_eff_weight_with_track_trigger:B_s0_LOKI_DTF_CTAU/0.2997>>hprof", cut, "goff");
	//DecayTree->Draw("1/B_velo_eff_weight_with_trigger:time>>hprof", "sWeights_ipatia*(triggerDecisionUnbiased==1)", "goff");
	DecayTree->Draw("1/B_velo_eff_weight_with_track_trigger:time>>hprof", "sWeights_ipatia*(triggerDecisionBiasedExcl==1)", "goff");
	...
	*/
	...
	hprof->GetYaxis()->SetTitle("<1/weight>");
	hprof->GetYaxis()->SetTitleOffset(0.95);
	hprof->SetMinimum(0.5);
	//hprof->SetMinimum(0.97);
	//hprof->Fit(pol, "R");
	c->SaveAs("MC2012_sim08e_efficiency_velo.pdf");
	//c->SaveAs("data_efficiency_velo_with_sWeights.pdf");
}
