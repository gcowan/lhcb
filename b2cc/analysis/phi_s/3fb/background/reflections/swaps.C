...
void swaps::Loop()
{
	...
	//for (Long64_t jentry=0; jentry<1000;jentry++) {
		...
		if ( !(sel_cleantail==1&&sel==1&&(triggerDecisionUnbiased==1||triggerDecisionBiasedExcl==1)&&time>2.&&time<14&&sigmat>0&&sigmat<0.12
...
//			&&(Kminus_pidK-Kminus_pidp)>-5
		...
		if (mass > 5420) { 
			mBsKpi->Fill(BpiK.M()/1000.);
			mKpi->Fill(piK.M()/1000.);
			}
		if (mass < 5330) 
			mBspiK->Fill(BKpi.M()/1000.);
		if (mass < 5330) 
