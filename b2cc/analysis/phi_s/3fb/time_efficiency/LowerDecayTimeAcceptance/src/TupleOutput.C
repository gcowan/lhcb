...
void DumpData(TTree* tree_, TTree* tree_excl, TString const& inputfilename, particleProperties* prop, Bool_t MCtruth = kFALSE, TString suffix = "", Bool_t IPzWeight = kFALSE)
{
  ...
  //Float_t B_CTAU, B_CTAUERR, B_M, B_IPzWeight, time_scale;
  //Int_t B_Hlt1TrackMuon_TOS,B_Hlt1TrackAllL0_TOS, B_Hlt1DiMuonHighMass_TOS;
  //Int_t B_Hlt2DiMuonJPsi_TOS, B_Hlt2DiMuonDetachedJPsi_TOS;
  
  tree->SetBranchAddress(prop->tupleName+"_LOKI_MASS_JpsiConstr", &B_M);
  tree->SetBranchAddress(prop->tupleName+"_LOKI_DTF_CTAU", &B_CTAU);
  tree->SetBranchAddress(prop->tupleName+"_LOKI_DTF_CTAUERR", &B_CTAUERR);
  
  tree->SetBranchAddress(prop->tupleName+"_Hlt1TrackMuonDecision_TOS", &B_Hlt1TrackMuon_TOS);
  tree->SetBranchAddress(prop->tupleName+"_Hlt1TrackAllL0Decision_TOS", &B_Hlt1TrackAllL0_TOS);
  tree->SetBranchAddress(prop->tupleName+"_Hlt1DiMuonHighMassDecision_TOS", &B_Hlt1DiMuonHighMass_TOS);
  tree->SetBranchAddress(prop->tupleName+"_Hlt2DiMuonJPsiDecision_TOS", &B_Hlt2DiMuonJPsi_TOS);
  tree->SetBranchAddress(prop->tupleName+"_Hlt2DiMuonDetachedJPsiDecision_TOS", &B_Hlt2DiMuonDetachedJPsi_TOS);
  if(IPzWeight){
  ...
  for (Long64_t ientry=0; ientry<nentries_excl;ientry++) {
    ...
    B_M = B_M;
    B_CTAU = B_CTAU/0.299792458;
    B_CTAUERR = B_CTAUERR/0.299792458;
    ...
    TS0 = B_Hlt2DiMuonJPsi_TOS>0 && B_Hlt1DiMuonHighMass_TOS>0;
    TS1 = B_Hlt2DiMuonDetachedJPsi_TOS>0 && B_Hlt1DiMuonHighMass_TOS>0;
  ...
  for (Long64_t ientry=0; ientry<nentries;ientry++) {
    ...
    TS0 = B_Hlt2DiMuonJPsi_TOS>0 && B_Hlt1DiMuonHighMass_TOS>0;
    TS1 = B_Hlt2DiMuonDetachedJPsi_TOS>0 && B_Hlt1DiMuonHighMass_TOS>0;
    TS2 = B_Hlt2DiMuonDetachedJPsi_TOS>0 && (B_Hlt1DiMuonHighMass_TOS>0||B_Hlt1TrackMuon_TOS>0||B_Hlt1TrackAllL0_TOS>0);
    TS2bis = (B_Hlt2DiMuonDetachedJPsi_TOS>0) && (B_Hlt1TrackAllL0_TOS>0||B_Hlt1TrackMuon_TOS>0) && !(B_Hlt1DiMuonHighMass_TOS>0);
