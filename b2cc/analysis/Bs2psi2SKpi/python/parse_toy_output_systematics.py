...
for l in fractions:
	...
	width = (bins[1] - bins[0])
	plot_binned_data(ax[count % dim, count / dim], bins, hist, histtype="step")
	ax[count % dim, count / dim].set_xlabel(label[count])
	ax[count % dim, count / dim].text(0.05, 0.9, "mean = %0.3f +- %0.3f" % (np.average(l), np.std(l)/np.sqrt(len(l))) , va='center', transform=ax[count % dim, count / dim].transAxes)
	ax[count % dim, count / dim].text(0.05, 0.8, "std dev = %0.3f + - %0.3f" % (np.std(l), np.std(l)/np.sqrt(2*(len(l)-1))), va='center', transform=ax[count % dim, count / dim].transAxes)
	ax[count % dim, count / dim].xaxis.set_major_formatter(ticker.FormatStrFormatter('%0.3f'))
	if True: ax[count % dim, count / dim].text(0.05, 0.7, "syst = %0.3f" % np.sqrt(np.average(l)**2 + np.std(l)**2), va='center', transform=ax[count % dim, count / dim].transAxes)
	#ax[count % dim, count / dim].plot(bins, y, 'r--', linewidth=2)
	count += 1
