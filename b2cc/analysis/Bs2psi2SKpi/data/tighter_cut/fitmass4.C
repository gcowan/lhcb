...
void addBlurb(TCanvas* can){
  ...
  myLatex->DrawLatex(0.8, 0.85,"LHCb");
...
void fitmass4(std::string filename = "selected_candidates_with_correct_angles.root", std::string massbr = "B_s0_Mass_JpsiConstr",  bool plotZoom = false,  double minmass = 5200, double maxmass = 5500, double reflectGuess = 269 , double nlb = 71,   double themax = 280 , double themax2 =12000, int nbins = 50){
 ...
 TTree* tree = (TTree*)theFile->Get("tuple");
 tree->Print();
 ...
 RooRealVar mkstar("m23", "mkstar", 1300., 2000.);
 //RooRealVar cosTheta2("cosTheta2", "cosTheta2", -1.0, -0.6);
 //RooRealVar cosTheta2("cosTheta2", "cosTheta2", -0.6, -0.2);
 //RooRealVar cosTheta2("cosTheta2", "cosTheta2", -0.4, 0.2);
 //RooRealVar cosTheta2("cosTheta2", "cosTheta2", 0.2, 0.6);
 RooRealVar cosTheta2("cosTheta2", "cosTheta2", 0.6, 1.0);
 RooDataSet data("mass_DS", "mass_DS", RooArgSet(m, mkstar, cosTheta2),Cut(theCut),Import(*tree));   
 
 int ntot = data.numEntries();

 RooRealVar mx("mBd","mBd", 5279, 5270,5285);  //mx.setConstant(true);

 RooRealVar dm("dm","dm",87.6, 80, 95); 
 ...
 RooRealVar taub("taub","taub",-0.0011, -0.1,0.1); //taub.setConstant(true); 
 RooGaussian tauconstraint("tauConstr","tauConstr",taub,RooConst(-0.0011),RooConst(0.0004)) ;
 RooExponential bg("back","exp",m,taub);
 // RooRealVar s2("sigma2","sigma2",8.32,0, 15); //s2.setConstant(true);
 
 RooRealVar scale("scale", "scale", 1, 0.9, 1.3); 
 RooFormulaVar s2("sigma2", "sigma2", "7.48*scale",scale); 
 RooFormulaVar s("sigma", "sigma", "4.89*scale",scale); 
 //scale.setConstant(true);

 RooFormulaVar s3("sigma3", "sigma3", "sigma*1.06",s);
 RooFormulaVar s4("sigma4", "sigma4", "sigma2*1.06",s2);

 RooRealVar st("sigmat", "sigmat", 17.8, 7., 30); st.setConstant(true);
 RooGaussian gauss1("gauss1","gauss(x,mean,sigma)",m, mx,st); 
 RooGaussian gauss2("gauss2","gauss(x,mean,sigma)",m, mBs,st); 

 RooRealVar sghost("sghost", "sghost", 13.7, 0., 30); sghost.setConstant(true); 
 ...
 RooRealVar a("a", "a", 2.4, 0., 4); a.setConstant(true);
 ...
 RooRealVar a2("a2", "a2", 4.59, 0., 4); a2.setConstant(true);
 ...
 RooRealVar f("f","f",0.75, 0.65,0.85); f.setConstant(true); 
 ...
 RooRealVar rm("rm","rm", 5287., 5250,5420);  //rm.setConstant(true); // 4.2
 RooRealVar rs("rsigma","rsigma",232.,0, 400);  rs.setConstant(true); // 47
 RooRealVar rs2("rsigma2","rsigma2",7.5,0, 200);//rs2.setConstant(true); //2.4 
 RooBifurGauss bif("bif","bif",m,rm,rs,rs2);
 RooGaussian rsconstraint2("rsConstr2","rsConstr2",rs2,RooConst(7.5),RooConst(2.)) ;

 RooGaussian rmconstraint("rmConstr","rmConstr",rm,RooConst(5287),RooConst(3.3)) ;


 // reflection2
 RooRealVar rrm("rrm","rrm",  5334   , 5250,5500);  rrm.setConstant();
 RooRealVar rrs("rrsigma","rrsigma",12.4,0, 200);  rrs.setConstant(true);
 RooRealVar rrs2("rrsigma2","rrsigma2",110,0, 200);  rrs2.setConstant(true);
 ...
 // reflection2
 RooRealVar lrm("lrm","lrm",   5314   , 5100,5500);  lrm.setConstant(true);
 RooRealVar lrs("lrsigma","lrsigma",1000,0, 2000);  lrs.setConstant(true);
 RooRealVar lrs2("lrsigma2","lrrsigma2",42.8 ,0, 200);  lrs2.setConstant(true);
 ...
 RooRealVar nsigBs("NsigBs","back fraction", 330, 0., data.numEntries()); //nsigBs.setConstant(true);
 RooRealVar nback("Nbkg","back fraction",data.numEntries()/4.0, 0., data.numEntries());
 RooRealVar nref("Nref","back fraction", reflectGuess , 0., reflectGuess*100);// nref.setConstant(true);
 RooRealVar nref2("Nref2","back fraction", 12.3 , 0., data.numEntries()) ; nref2.setConstant(true); //Bs -> psi(2S) pipi
 ...
 RooRealVar nref7("Nref7","back fraction", 4. , 0., data.numEntries()) ; nref7.setConstant(true); //Lambdab
 ...
 RooGaussian nnrefconstraint("nrefConstr","nrefConstr",nref,RooConst(reflectGuess),RooConst(67)) ;
 ...
 RooFitResult* result = model.fitTo(data,Extended(),Save(), ExternalConstraints(RooArgSet( rsconstraint2, rmconstraint,nnrefconstraint)));// ,tauconstraint))); 
 ...
 xachse->SetTitleSize(0.07);
 yachse->SetTitleSize(0.07);
 ...
 pad1->cd();
   ...
   xachse2->SetTitleSize(0.07);
   yachse2->SetTitleSize(0.07);
   yachse2->SetTitleOffset(0.8);
   xachse2->SetTitleOffset(1.1);
   xachse2->SetTitle("");
   yachse2->SetTitle("");
 ...
 model.plotOn(frame3, Components(bg) , LineStyle(1), FillColor(92),LineColor(92), DrawOption("FL") );
 model.plotOn(frame3, Components(RooArgSet(bg, bif,bsbdref,lrbif,erbif)) ,LineStyle(1001) , FillStyle(3365), FillColor(38),LineColor(38), DrawOption("FL") );
 model.plotOn(frame3, Components(RooArgSet(bg, bif,bsbdref,lrbif,erbif)) ,LineColor(38), DrawOption("L") );
 model.plotOn(frame3, Components(bg) , LineStyle(1), FillColor(92),LineColor(92), DrawOption("FL") );
   ...
   frame3->SetXTitle(""); 
   frame3->SetYTitle(""); 
   frame3->SetXTitle("M(#psi(2S) K^{+}#pi^{#font[122]{-}}) [MeV/#font[12]{c}^{2}]");
