...
void plot_resolution_sim_2g(){
	...
	RooRealVar *mdau1 = new RooRealVar("mdau1", "dimuon mass",3630.0,3740.0, "MeV");
	RooRealVar *hlt2_unbiased = new RooRealVar("hlt2_unbiased", "hlt2_unbiased",-10.0,10.0);
	RooRealVar *muplus_PT = new RooRealVar("muplus_PT", "muplus_PT",0.0,100000.0);
	RooRealVar *muminus_PT = new RooRealVar("muminus_PT", "muminus_PT",0.0,100000.0);
	...
	RooArgList *fitArgs = new RooArgList(*time, *mdau1, *hlt2_unbiased, *muplus_PT, *muminus_PT, *sigmat, *sweight);
	TCut cut1 = "(hlt2_unbiased==1)&&(muplus_PT>1900&&muminus_PT>1900)";

	time->setRange("res",-0.4, 0.4);
	time->setRange("long",-1.0, 8.0);
	...
	RooRealVar *mean_g1 = new RooRealVar("g1 #mu","mean of g1",0.00,-0.1, 0.2);
	...
	RooPlot* frame0 = mdau1->frame(Bins(60),Title("m(mumu)"));
	data1->plotOn(frame0);
	TCanvas* c0 = new TCanvas("mdau1_plot","sim",600,800) ;
	gPad->SetLeftMargin(0.15) ; frame0->Draw() ;
	c0->SaveAs("plot_data_mdau1.pdf");
	...
	pad12->Draw();
	pad22->Draw();
	pad12->cd() ; gPad->SetLogy(); gPad->SetLeftMargin(0.15) ; frame2->GetYaxis()->SetTitleOffset(1) ; frame2->Draw() ;
