...
void swaps::Loop()
{
	...
	TH1D * mBs  = new TH1D("mJpsiKK", "mJpsiKK", 100, 4.20, 5.55);
	...
	TH1D * mBsKpi = new TH1D("upper_Bs_sideband", "mJpsiKpi", 40, 5.1, 5.7);
	TH1D * mBspiK = new TH1D("lower_Bs_sideband", "mJpsipiK", 40, 5.1, 5.7);
	TH1D * mJpsi_ = new TH1D("mJpsi", "mJpsi", 100, 3.63, 3.73);
	TH1D * mJpsi_constr = new TH1D("mJpsi_constr", "mJpsi", 100, 3.63, 3.73);
	...
	//for (Long64_t jentry=0; jentry<1000;jentry++) {
		...
		TLorentzVector KplusWrong(Kplus_PX, Kplus_PY, Kplus_PZ, sqrt(Kplus_PX*Kplus_PX+Kplus_PY*Kplus_PY+Kplus_PZ*Kplus_PZ + mmu*mmu));
		TLorentzVector KminusWrong(Kminus_PX, Kminus_PY, Kminus_PZ, sqrt(Kminus_PX*Kminus_PX+Kminus_PY*Kminus_PY+Kminus_PZ*Kminus_PZ + mmu*mmu));
		...
		TLorentzVector Jpsi_swap1 = muplus  + KminusWrong;
		...
		TLorentzVector Bminus = Jpsi_constr + Kminus;
		TLorentzVector BpiK = Jpsi_constr + piK;
		//mKK->Fill(KK.M()/1000.);
		//mBs->Fill(Bplus.M()/1000.);
		//mBs->Fill(Bminus.M()/1000.);
		//massHisto->Fill(mass/1000.);
        cout << Jpsi_swap1.M() << endl;
        mJpsi_->Fill(Jpsi_swap1.M()/1000.);
		mJpsi_->Fill(Jpsi_swap2.M()/1000.);
		//mJpsi_constr->Fill(Jpsi_constr.M()/1000.);
	    //mBsKpi->Fill(BKpi.M()/1000.);
		//mKpi->Fill(Kpi.M()/1000.);
		//mBspiK->Fill(BpiK.M()/1000.);
	}
	...
	//mBs->Draw();
	///mBs->SetTitle("");
	///mBs->GetXaxis()->SetTitle("m(#psi(2S) K^{+}) [GeV/c^{2}]");
	mJpsi_->Draw();
	mJpsi_->SetTitle("");
	mJpsi_->GetXaxis()->SetTitle("m(h#rightarrow #mu,#mu) [GeV/c^{2}]");
	c->SaveAs("mass_mumu_hadron_swap.pdf");
    /*
	...
	mKp->GetXaxis()->SetTitle("m(K#pi) [GeV/c^{2}]");
	c->cd(4);
	massHisto->Draw();
