...
void fit(){
    float tau_Bd(1.525);
	//TFile *file = TFile::Open("/Disk/ecdf-nfs-ppe/lhcb/gcowan/Bs2psi2SKpi/B_s0/data/tighter_cut/tight-mva.root");
	//TFile *file = TFile::Open("/Disk/ecdf-nfs-ppe/lhcb/gcowan/Bs2psi2SKpi/B_s0/data/no_mva/signal_proton.root"); tau_Bd = 1.520; // signal data
	TFile *file = TFile::Open("/Disk/ecdf-nfs-ppe/lhcb/gcowan/Bs2psi2SKpi/B_s0/data/no_mva/BdKpi-mc_proton_nomva.root"); tau_Bd = 1.525; // signal MC
	...
	RooArgList *fitArgs = new RooArgList(*ctime, *hlt1_unbiased, *hlt2_unbiased, *sigmat, *sweight, *mass);
    ...
    //TCut cut1 = "(B_s0_Hlt1DiMuonHighMassDecision_TOS==1 && B_s0_Hlt2DiMuonDetachedPsi2SDecision_TOS==1)";
	...
	RooRealVar *sf1 = new RooRealVar("sf1", "sf1", 0.043);
	...
	RooRealVar *tau2 = new RooRealVar("tau2", "tau2", tau_Bd);
	...
	RooBDecay *bdc12 = new RooBDecay("bdc12","bdc12",*time,*tau2,*DG,RooConst(1),RooConst(0), RooConst(0), RooConst(0),RooConst(0), *gauss11, RooBDecay::SingleSided);
	...
	RooRealVar * beta    = new RooRealVar("beta", "beta", -0.05,-0.15,0.35);
	RooRealVar * beta_sq = new RooRealVar("beta_sq", "beta_sq", 0.);
	RooRealVar * p0   = new RooRealVar("p0", "p1", 0.);
	RooRealVar * p1   = new RooRealVar("p1", "p1", 2.,0.,5.);
	RooRealVar * p2   = new RooRealVar("p2", "p2", 2.,0.,5.);
    ...
    RooEffProd * modelEff = new RooEffProd("modelEff","model with efficiency", *bdc12, *eff);

	gROOT->ProcessLine(".x ~/lhcb/lhcbStyle.C");
    // Generate some events from the lifetime distribution
    RooDataSet * theory_data = bdc12->generate(*time, 1e6);
    const int nbins(40);
    double bins[nbins+1];
	...
	RooPlot * ratio_frame = time->frame(Binning(*binning));

	RooFitResult *fitresult = modelEff->fitTo(*data1,Save(kTRUE), NumCPU(6),SumW2Error(kFALSE)); 
