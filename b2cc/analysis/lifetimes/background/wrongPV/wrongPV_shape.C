...
void wrongPV_shape::Loop()
{
	...
	TFile * file = TFile::Open("time_wrongPV.root", "RECREATE");
	TNtuple * tuple = new TNtuple("DecayTree","DecayTree", "time:timeWrongPV");
	...
	double PVx_previous(0.);
	double PVy_previous(0.);
	double PVz_previous(0.);
	...
	for (Long64_t jentry=0; jentry<nentries;jentry++) {
		...
		TVector3 pv(PVx_previous, PVy_previous, PVz_previous);
		TVector3 decayvertex(Bu_decayVertex_X, Bu_decayVertex_Y, Bu_decayVertex_Z);

		mass = B.M();
		momentum = B.P();
		decaylength = (pv - decayvertex).Mag();
		decaytime = decaylength	* mass / momentum;
