...
void PhaseC( std::string filename = "bsdsrho.root", int nToGen = 1e7*2.3 ) {
	...
	for (Int_t n=0;n<nToGen;n++) {
	  //	   Double_t weight = event.Generate();
	   ...
	   TLorentzVector ss3pHadron2 = *subevent3.GetDecay(1); 
           ...
           masses2[1] = pick(phidata,ran,std::string("m1"));
           masses2[0] = pick(rhodata,ran,std::string("m2"));
	   //std::cout << masses2[1] << std::endl;
   	   bool okphi =  subevent.SetDecay (pHadron1, 2, masses2);
	   if (!okphi) continue;
