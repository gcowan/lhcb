...
void fitBack4(std::string filename = "reflection_Info2.root", bool paramOn = false, double minmass = 5200, double maxmass = 5350, int nbins = 50){
 ...
 */
 ...
 RooRealVar mean("mean", "Mean", 0.5*(minmass+maxmass), minmass-200, maxmass);
 RooRealVar mean2("mean2", "Mean2", 0.5*(minmass+maxmass), minmass-50, maxmass);
 ...
 RooNovosibirsk bg1("novo", "novo(x,mean,sigma,tail)", m, mean, s, tail);
 RooNovosibirsk bg2("novo2", "novo(x,mean,sigma,tail)", m, mean2, s2, tail);

 RooCBShape cb("crystalball","cb(x,mean,sigma)",m, mean,s,a,n);
 RooBifurGauss bif("bif","bif",m, mean,s,s2);

 RooAddPdf bgmodel("bgmodel","bgmodel",RooArgList(bg1,bg2), f);

 bgmodel.fitTo(data,NumCPU(4)); 
 ...
 bgmodel.plotOn(mframe, LineColor(kRed));
 mframe->Draw();
 addBlurb(c1);
