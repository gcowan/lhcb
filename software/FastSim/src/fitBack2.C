...
void fitBack2(std::string filename = "reflection_Info2.root", bool paramOn = false, double minmass = 5200, double maxmass = 5350, int nbins = 50){
 ...
 */
 RooRealVar aMean("amean", "amean",  5260  , 5200, 5350); //aMean.setConstant(true);
 RooRealVar p("p", "p",  -0.03  , -1, 1);  p.setConstant(true);
 RooRealVar ac("ac", "ac",  10.  , -30, 30); //ac.setConstant(true);
 RooArgusBG argus("argus", "argus", m, aMean, ac,p);
 ...
 phi_sig->plotOn(mframe, LineColor(kRed));
 mframe->Draw();
 addBlurb(c1);
